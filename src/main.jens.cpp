...
#if (ORIGINALMENU == 0)
typedef enum { cmSubdivision = 1, cmForwardDifferencing = 2, cmSampling = 3 } CurveModels;
typedef enum { smPhong = 1, smGouraud = 2 } ShadingModels;
char                   cur_figure         = figure;
int                    forward_diff_steps = 200;
CurveModels            cur_curve_model    = cmSubdivision;
MyMathTypes::real_type rot_step           = 10.0;
MyMathTypes::real_type cur_z_rot          = 45.0;
MyMathTypes::real_type cur_y_rot          = 0.0;
ShadingModels          cur_shading_model  = smPhong;
#endif
...
{
    MyMathTypes::real_type z_rotation_angle = cur_z_rot * M_PI /  180.0;
    MyMathTypes::real_type y_rotation_angle = cur_y_rot * M_PI /  180.0;
    render_pipeline.state().model()         = Z_Rotate(z_rotation_angle) * Y_Rotate(y_rotation_angle);
    render_pipeline.state().inv_model()     = Inv_Z_Rotate(z_rotation_angle) * Inv_Y_Rotate(y_rotation_angle);
}
...
void usage()
{
    ...
    std::cout << "\tCamera movement:" << std::endl << std::flush;
    std::cout << "\t----------------" << std::endl << std::flush;
    std::cout << "\t. : Reset       " << std::endl << std::flush;
    std::cout << "\tk : Up          " << std::endl << std::flush;
    std::cout << "\tj : Down        " << std::endl << std::flush;
    std::cout << "\th : Left        " << std::endl << std::flush;
    std::cout << "\tl : Right       " << std::endl << std::flush;
    std::cout << "\ty : Up-left     " << std::endl << std::flush;
    std::cout << "\tu : Up-right    " << std::endl << std::flush;
    std::cout << "\tb : Down-left   " << std::endl << std::flush;
    std::cout << "\tn : Dow-right   " << std::endl << std::flush;
    std::cout << std::endl << std::flush;

    std::cout << "\tCurve Model:" << std::endl << std::flush;
    std::cout << "\t------------" << std::endl << std::flush;
    std::cout << "\te : Use Subdivision          " << std::endl << std::flush;
    std::cout << "\tf : Use Forward Differencing " << std::endl << std::flush;
    std::cout << "\tg : Use Sampling             " << std::endl << std::flush;
    std::cout << std::endl << std::flush;
    
    std::cout << "\tShading Model:       " << std::endl << std::flush;
    std::cout << "\t--------------       " << std::endl << std::flush;
    std::cout << "\tm : Gouraud shading  " << std::endl << std::flush;
    std::cout << "\to : Phong shading    " << std::endl << std::flush;
    std::cout << std::endl << std::flush;
    
    std::cout << "\tDraw General Parametric Surfaces:" << std::endl << std::flush;
    std::cout << "\t---------------------------------" << std::endl << std::flush;
    std::cout << "\ta : Klein Bottle"                  << std::endl << std::flush;
    std::cout << "\ts : Klein Bottle interior (does not work)"         << std::endl << std::flush;
    std::cout << "\tx : Phong Surface"                 << std::endl << std::flush;
    std::cout << std::endl << std::flush;

    std::cout << "\tDraw Bezier Surfaces:"             << std::endl << std::flush;
    std::cout << "\t---------------------"             << std::endl << std::flush;
    std::cout << "\tt : UTAH Teapot"                   << std::endl << std::flush;
    std::cout << "\tz : Rocket"                        << std::endl << std::flush;
    std::cout << "\tv : Sailboat"                      << std::endl << std::flush;
    std::cout << "\tp : Pain"                          << std::endl << std::flush;
    std::cout << std::endl << std::flush;
#endif
...
/*  // TEST00 : 
    ...
    {
        // Use Forward Differencing
        cur_curve_model = cmForwardDifferencing;
        figure = cur_figure;
        glutPostRedisplay();
    }
...
void RefreshMenu()
{
    ...
    glutChangeToMenuEntry( 9,  "   Reset camera", 9);
    glutChangeToMenuEntry(10,  "   Camera up", 10);
    glutChangeToMenuEntry(11,  "   Camera down", 11);
    glutChangeToMenuEntry(12,  "   Camera left", 12);
    glutChangeToMenuEntry(13,  "   Camera right", 13);
    glutChangeToMenuEntry(14,  "   Camera up-left", 14);
    glutChangeToMenuEntry(15,  "   Camera up-right", 15);
    glutChangeToMenuEntry(16,  "   Camera down-left", 16);
    glutChangeToMenuEntry(17,  "   Camera down-right", 17);
    glutChangeToMenuEntry(18,  "   Zoom in", 18);
    glutChangeToMenuEntry(19,  "   Zoom out", 19);
    
/*  // TEST00        
    glutChangeToMenuEntry(20,  "   Use Subdivision", 20);
    glutChangeToMenuEntry(21,  "   Use Forward Differencing", 21);
    glutChangeToMenuEntry(22,  "   Use Sampling", 22);
*/  // TEST00

    glutChangeToMenuEntry(23,  "   Use Phong shading", 24);
    glutChangeToMenuEntry(24,  "   Use Gouraud shading", 24);
...
void SelectMenuItem(int value)
{
    switch (value) {
        ...
        case 23:
            // Use Sampling
            RefreshMenu();
            glutChangeToMenuEntry(23, "-> Use Phong shading", 23);
            figure = 'o';
            glutPostRedisplay();
            break;
        case 24:
...
void CreateMenu()
{
    ...
    glutAddMenuEntry("   Camera up", 10);
    glutAddMenuEntry("   Camera down", 11);
    glutAddMenuEntry("   Camera left", 12);
    glutAddMenuEntry("   Camera right", 13);
    glutAddMenuEntry("   Camera up-left", 14);
    glutAddMenuEntry("   Camera up-right", 15);
    glutAddMenuEntry("   Camera down-left", 16);
    glutAddMenuEntry("   Camera down-right", 17);
    glutAddMenuEntry("   Camera zoom in", 18);
    glutAddMenuEntry("   Camera zoom out", 19);

/*  // TEST00    
    glutAddMenuEntry("   Use Subdivision", 20);
    glutAddMenuEntry("   Use Forward Differencing", 21);
    glutAddMenuEntry("   Use Sampling", 22);
*/
    
    glutAddMenuEntry("   Use Phong shading", 23);
    glutAddMenuEntry("   Use Gouraud shading", 24);
